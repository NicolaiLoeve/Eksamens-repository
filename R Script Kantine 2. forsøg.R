...
df_counts2 <- df_kantobs2 %>%
  count(interval, name = "count")

# 3) Lav en komplet liste af alle minutter i perioden
all_intervals <- tibble(
  interval = seq(
    from = floor_date(min(df_kantobs2$timestamp), "1 minute"),
    to   = floor_date(max(df_kantobs2$timestamp), "1 minute"),
    by   = "1 min"
  )
)

# 4) Join og sæt manglende til 0
df_counts_full2 <- all_intervals %>%
  left_join(df_counts2, by = "interval") %>%
  mutate(count = replace_na(count, 0L))

# 5) Heatmap langs tidsaksen
ggplot(df_counts_full2, aes(x = interval, y = 1, fill = count)) +
  geom_tile() +
...
scale_y_continuous(NULL, breaks = NULL) +
  ...
  scale_x_datetime(
    breaks = seq(min(df_counts_full2$interval),
                 max(df_counts_full2$interval),
                 by = "1 min"),
    date_labels = "%H:%M"
  ) +
  labs(
...
threshold <- mean(df_counts_full$count2)

df_counts_full2 <- df_counts_full2 %>%
  mutate(traveligt = count > threshold)

ggplot(df_counts_full2, aes(x = interval, y = count, fill = traveligt)) +
  geom_col() +
